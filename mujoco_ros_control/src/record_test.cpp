...
{
    // activate
    mj_activate(getenv("MUJOCO_KEY_PATH"));
...
int main(int argc, const char** argv)
{
    ...
    cam.fixedcamid = 0;
    mjrRect my_viewport = {0, 0, W, H};

    // alright so these are right.
    float extent = m->stat.extent;
    float znear = m->vis.map.znear * extent;
    float zfar = m->vis.map.zfar * extent;

    // also testing fovy.
    std::cout << "m->stat.extent " << m->stat.extent << "\n";
    std::cout << "m->vis.map.znear " << m->vis.map.znear << "\n";
    std::cout << "m->vis.map.zfar " << m->vis.map.zfar << "\n";
    std::cout << "my znear " << znear << "\n";
    std::cout << "my zfar " << zfar << "\n";
    std::cout << "m->cam_fovy" << m->cam_fovy[0] << ", " << m->cam_fovy[1] << "\n";
    ...
    while( d->time<duration )
    {
        ...
        if( (d->time-frametime)>1/fps || frametime==0 )
        {
            ...
            // DO CV STUFF
            std::string filenum = std::to_string(framecount);
            padTo(filenum, 10);
            std::string filepath = "/home/abba/test_data/" + filenum + ".png";
            cv::Mat img(cv::Size(W, H), CV_8UC3, rgb);
            bool result = cv::imwrite( filepath, img );

            // looks good. let's print some buffer values.
            // for( int r=0; r<H; r++ )
            //     for( int c=0; c<W; c++ )
            //     {
            ...
            float max_depth = -100000000.0;
            float min_buff =  100000000.0;
            float max_buff = -100000000.0;

            for( int r=0; r<H; r++ )
                for( int c=0; c<W; c++ )
                {
                  // float real_depth = depth[r*W+c];
                  float real_depth = znear / (1.0f - depth[r*W+c] * (1.0f - znear / zfar));
                  if(real_depth < min_depth)
                    min_depth = real_depth;

                  if (real_depth > max_depth)
                    max_depth = real_depth;

                  if (depth[r*W+c] > max_buff)
                    max_buff = depth[r*W+c];

                  if (depth[r*W+c] < min_buff)
                    min_buff = depth[r*W+c];
